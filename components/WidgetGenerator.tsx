    ...
    <div className="p-10 h-full overflow-y-auto custom-scrollbar bg-[var(--background)] transition-colors duration-500">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        <div className="space-y-8">
          <h3 className="text-xl font-black text-[var(--foreground)] tracking-tight px-2">Modules Disponibles</h3>
              ...
              return (
                <div 
                  ...
                  className={clsx(
                    ...
                    widget.enabled && "border-[var(--accent)] shadow-lg shadow-[var(--accent)]/5"
                  )}
                >
                  <div className="flex items-center gap-6">
                    <div className={clsx(
                      "w-16 h-16 rounded-[22px] flex items-center justify-center transition-all shadow-sm border border-white/10",
                      widget.enabled ? "bg-[var(--accent)] text-white shadow-[var(--accent)]/30" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400"
                    )}>
                    ...
                    <div>
                      <h4 className="text-lg font-black text-[var(--foreground)] flex items-center gap-3 tracking-tight">
                        {widget.name}
                        {widget.isBeta && <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-500 text-[9px] font-black uppercase px-3 py-1 rounded-full tracking-widest border border-[var(--card-border)]">Bêta</span>}
                      </h4>
                      <p className="text-sm text-[var(--muted)] font-medium mt-1 leading-relaxed max-w-sm">{widget.description}</p>
                    </div>
                  </div>
                  ...
                  {!widget.isBeta && (
                    <button 
                      ...
                      className={clsx(
                        ...
                        widget.enabled ? "bg-[var(--accent)]" : "bg-zinc-200 dark:bg-zinc-700"
                      )}
                    >
                      <span
                        className={clsx(
        ...
        <div className="space-y-8">
          ...
          <div className="card-premium p-10 overflow-hidden relative min-h-[500px] flex flex-col">
            ...
            <p className="text-sm text-[var(--muted)] font-medium mb-12 max-w-md relative z-10 leading-relaxed">
              Le back-office est directement relié par GPS à l&apos;application de vos citoyens. Dès qu&apos;un habitant ouvre l&apos;application dans votre commune, les modules activés s&apos;affichent instantanément.
            </p>
            ...
            <div className="flex-1 flex flex-col md:flex-row items-center gap-12 relative z-10">
              ...
              <div className="flex-1 w-full bg-[var(--accent)]/5 border border-[var(--accent)]/10 rounded-[40px] p-10 relative flex items-center justify-center">
                ...
                <div className="relative z-10 flex flex-col items-center justify-center text-[var(--accent)] text-center bg-[var(--card)]/80 backdrop-blur-xl rounded-[32px] p-8 shadow-2xl border border-white/20">
                  <span className="font-black text-lg tracking-tight mb-1">Secteur Ville</span>
                  <span className="text-[10px] font-black uppercase tracking-widest opacity-60">Liaison Active</span>
                </div>
              </div>

              <div className="text-[var(--accent)] opacity-40 transform md:-rotate-90 md:mx-6">
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="animate-bounce">
              ...
              <div className="flex-1 w-full flex flex-col items-center">
                <div className="w-32 h-64 bg-zinc-900 dark:bg-black rounded-[48px] shadow-2xl border-8 border-zinc-800 dark:border-zinc-900 p-3 flex flex-col gap-3 relative overflow-hidden shadow-[var(--accent)]/20">
                  <div className="w-full bg-zinc-100 dark:bg-zinc-800 h-8 rounded-2xl flex items-center px-3 mb-2">
                    ...
                    <span className="text-[7px] font-black text-zinc-800 dark:text-zinc-200 uppercase tracking-widest">Municip&apos;All</span>
                  </div>
                  {activeWidgets.length > 0 ? (
                    <div className="space-y-2">
                      {activeWidgets.map(widget => {
                        const Icon = widget.icon;
                        return (
                          <div key={widget.id} className="bg-[var(--accent)]/10 border border-[var(--accent)]/20 rounded-xl p-2 flex items-center gap-2">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center opacity-30 gap-2">
                      <span className="text-[10px] text-[var(--muted)] font-black uppercase tracking-widest text-center px-2">Offline</span>
                    </div>
                  )}
                ...
                <div className="mt-8 flex items-center gap-3 bg-emerald-500/10 px-5 py-2.5 rounded-full border border-emerald-500/20 shadow-sm animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Connecté</span>
                </div>
              </div>
